...
Open the root URL in a browser to use the API testing console. It loads the available personas and authors, sends recommendation or specific-mode requests directly from the page, and presents the response as an overview with the selected persona's writing style and the selected author's full name, age, and job title, plus expandable guidance, keyword research, Markdown template, and raw JSON sections. Programmatic clients can request JSON from `GET /` with `Accept: application/json`.
...
Retrieve the available author IDs and profile details with `GET /v1/authors`. The shared `config/authors.json` file provides the selected author record to every template type, including full name, age, and job title. John (`john`) is the default author, and Melissa (`melissa`) and Radovan (`radovan`) are available selections.
